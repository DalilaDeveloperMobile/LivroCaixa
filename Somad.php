...
//$sql = " SELECT  SUM(pastoresa), SUM(congregacaoa) , (SUM(pastoresa) - SUM(congregacaoa))  FROM formoferta";

$sql = " SELECT  (SELECT  SUM(pastoresa) from formoferta) , (SELECT SUM(congregacaoa) from formos) , ((SELECT  SUM(pastoresa) from formoferta) - (SELECT SUM(congregacaoa) FROM formos))  ";
...
<legend><h3>TOTAL DE ENTRADAS E SAIDAS OFERTA </h3></legend>
